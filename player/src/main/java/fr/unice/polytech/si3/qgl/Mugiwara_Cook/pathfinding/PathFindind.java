...
public class PathFindind {
    ...
    /**    public boolean findPath(Node start, Node end) {  //return null si pas de chemin
     ...
     this.path = new ArrayList<>();
     if (aRename2(end)) return false;
     nodeToCheckpoint();
     return true;
     }

     private boolean aRename2(Node end) {
     while (true) {
     if (openSet.size() > 0) {
     int lowerIndex = 0;
     lowerIndex = getLowerIndex(lowerIndex);
     Node current = openSet.get(lowerIndex);
     if (current == end) {
     color(current);
     endAlgo = true; //????
     ...
     private int getLowerIndex(int lowerIndex) {
     for (int i = 0; i < openSet.size(); i++) {
     if (openSet.get(i).getF() < openSet.get(lowerIndex).getF())
     lowerIndex = i;
     }
     ...
     private void aRename(Node end, Node current, ArrayList<Node> voisin) {
     for (Node i : voisin) {
     if (!closeSet.contains(i) && !i.isWall()) {
     int tempG = current.getG() + 1;
     boolean newPath = false;
