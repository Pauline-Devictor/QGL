...
public class CollisionDetector2 {
    ...
    public void gris(Reef reef, List<List<Node>> map) {
        if (reef.getShape().getType().equals("rectangle")) {
            this.grisRectangleReef(reef, map);
        }
    }

    public void grisRectangleReef(Reef reef, List<List<Node>> map) {
...
//            System.out.println("Point: " + point.getY() + "+" + point.getX());
            try {
                reefcoinNode.add(this.closetNodeFromPoint(point, map));
            }catch (Exception e){
        ...
        for (int i = 0; i < sizecoin; i++) {
            this.grisline(reefcoinNode.get(i), reefcoinNode.get((i + 1) % (sizecoin)), map);
        }
    }
    ...
    public Node closetNodeFromPoint(Point point, List<List<Node>> map) throws Exception {
